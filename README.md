...
- **交互式 OAuth 登录** — 官方有交互式 OAuth 授权，本项目的 `--as user` 需要手动粘贴 token
- **Event WebSocket 订阅** — 官方支持实时事件推送（长连接），本项目是 CLI 工具，不做常驻服务
- **Whiteboard 图表渲染** — 官方支持 Mermaid → 飞书白板，本项目不涉及

~~任务子任务和提醒~~ 已补（create-subtask / list-subtasks / --reminder / add-follower）。~~文档媒体插入~~ 已补（upload-image / upload-file）。

剩余差距对大多数 Agent 场景影响有限，但如果重度依赖邮件收发，官方 CLI 更合适。
