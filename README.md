...
AI Agent 接入飞书，主流方案要吃掉 15,000+ tokens 的上下文窗口——光加载工具列表就烧完了，还没开始干活。本项目把这个数字压到 **655 tokens**。同样的 125 个 API 端点，29 类能力，上下文成本只有官方方案的 **4%**。
...
- **邮件收发** — 官方支持完整的邮件 CRUD（收/发/草稿/回复/标签），本项目只有邮件组查询
- **任务子任务和提醒** — 官方支持 subtask、reminder、follower，本项目只有基础 task CRUD
- **文档媒体插入** — 官方可以往文档里插入图片/文件，本项目的 doc write 只接受 Markdown 文本
- **用户 OAuth 登录流** — 官方有交互式 OAuth 授权，本项目的 `--as user` 需要手动粘贴 token
- **Event WebSocket 订阅** — 官方支持实时事件推送（长连接），本项目是 CLI 工具，不做常驻服务
- **Whiteboard 图表渲染** — 官方支持 Mermaid → 飞书白板，本项目不涉及
